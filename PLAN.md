...
- Plaintext modulus `p = 2^8 = 256`  → `Δ = q/p = 2^24`. One byte per Z_p cell.
...
- Approx error `ε = 4`. Fingerprint length `f = 8` bytes.

### Matrix layout (exactly reproduces CANS2026 Table 3 & Hao Table 5)
For `n` key-value pairs, value length `ℓ` bytes:
```
partition = ℓ + 8                       # byte-planes: 8-byte fingerprint ‖ ℓ value bytes
...
```
cell(col, row) = sorted_entry[ g ]   where g = col*dataRows + (row - ε - 1)
                 or ⊥ (0xFF) if g ∉ [0, n)
```
where `sorted_entry[g] = fingerprint8(key_g) ‖ value_g`.

Verified: m=10^6 → 32B:(6325,170·40)=25.3/27.2kB, 256B=65.0/77.1kB, 1024B=128.5/177.5kB.

### Row-KOPIR (SimplePIR, column-selection form)
Database `D` is a single `R × C` byte matrix, `R = rows·partition`, `C = columns`.
Public `A ∈ Z_q^{C×N}` sampled from a seed.
...
- **Recover**: `d = ans − h_s`; each entry rounds `Δ·plaintext + noise → plaintext`
  via `round(x) = (x + 2^23) >> 24  (mod 256)`. Yields column `col` = `rows`
  entries × `partition` bytes.
...
- Head-to-head configs: `m ∈ {10^6}`, `ℓ ∈ {32, 256, 1024} B`, N=1275, ε=4, p=256.

## Correctness checks
- End-to-end: build DB, query each present key → recovers correct value; absent key → ⊥.
- Noise margin: recovered `d` decodes with `|noise| ≪ Δ/2 = 2^23` at all configs.
</content>
